...
describe("id minting", () => {
  test("mints an id under the pursuit's prefix, and reports it on the committed op", () => {
    ...
    expect(op?.op).toBe("add_activity");
    const id = op?.op === "add_activity" ? op.id : "";
  ...
  test("the id carries no trace of the label, so correcting one does not orphan the other", () => {
    const result = normalize(EMPTY, [task("Ask Dana to play Thursday")]);
    if (!result.ok) throw new Error("unreachable");
    const id = result.value[0]?.op === "add_activity" ? result.value[0].id : "";
  ...
  test("two identical labels in one batch get distinct ids", () => {
    ...
    if (!result.ok) throw new Error("unreachable");
    const ids = result.value.flatMap((op) => (op.op === "add_activity" ? [op.id] : []));
  ...
  test("minting avoids ids already committed", () => {
    ...
    if (!result.ok) throw new Error("unreachable");
    const id = result.value[0]?.op === "add_activity" ? result.value[0].id : "";
    expect([...head.activities.keys()]).not.toContain(id);
  });
...
describe("$N resolution", () => {
  test("resolves an edge to ids minted earlier in the same batch", () => {
    ...
    if (!result.ok) throw new Error("unreachable");
    const minted = result.value.flatMap((op) => (op.op === "add_activity" ? [op.id] : []));
  ...
  test("resolves refs inside an activity spec, not only at the top level", () => {
    ...
    const awaitedSpec = awaited?.op === "add_activity" ? awaited.spec : null;
    // Every `$0` here means the first activity, so they all have to land on the id it minted.
    const sendInvite = result.value[0]?.op === "add_activity" ? result.value[0].id : "";
    expect(awaitedSpec?.on_timeout).toBe(sendInvite);
    expect(awaitedSpec?.deadline).toEqual({ after: sendInvite, duration: "48h" });
    expect(undo?.op === "add_activity" ? undo.spec.compensates : null).toBe(sendInvite);
  });
  ...
  test("resolves obviated_if.wait", () => {
    ...
    const quorum = result.value[1]?.op === "add_activity" ? result.value[1].id : "";
    expect(chase?.op === "add_activity" ? chase.spec.obviated_if : null).toEqual({
      wait: quorum,
      satisfied: true,
    });
  });
  ...
  test("rejects a ref to an op that minted no id", () => {
    expectRefusal(
      ...
      [
        task("A"),
        { op: "set_status", activity: "$0", status: "done", evidence_ref: "e" },
  ...
  test("rejects a committed id that does not exist — never invent an id", () => {
    expectRefusal(EMPTY, [{ op: "set_status", activity: "ghost", status: "done", evidence_ref: "e" }], "UNKNOWN_ACTIVITY");
    expectRefusal(seeded([task("A")]), [{ op: "add_edge", from: "a", to: "ghost" }], "UNKNOWN_ACTIVITY");
    expectRefusal(seeded([task("A")]), [{ op: "supersede_activity", activity: "a", by: "ghost" }], "UNKNOWN_ACTIVITY");
    expectRefusal(EMPTY, [task("A", { compensates: "ghost" })], "UNKNOWN_ACTIVITY");
  });

  test("a supersede with no replacement stays shaped that way", () => {
    const head = seeded([task("A")]);
    const result = normalize(head, [{ op: "supersede_activity", activity: "a" }]);
    if (!result.ok) throw new Error("unreachable");
    expect(result.value[0]).toEqual({ op: "supersede_activity", activity: nid(head, "a") });
  });
});
