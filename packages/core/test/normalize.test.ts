...
import { seeded, action, acceptEvent, resolveSlugs, nid } from "./fixtures.ts";
...
function normalize(graph: Graph, ops: AuthoredOp[]) {
  ...
  return normalizeBatch(
    graph,
    resolveSlugs(graph, parsed.value) as AuthoredOp[],
    "t",
    graph.version + 1,
  );
}
...
describe("id minting", () => {
  test("mints an id under the pursuit's prefix, and reports it on the committed op", () => {
    const result = normalize(EMPTY, [action("Ask Dana to play Thursday")]);
  ...
  test("is deterministic, which is what lets fold and mutate agree", () => {
    ...
    const once = normalize(EMPTY, [action("Ask Dana to play Thursday")]);
    const twice = normalize(EMPTY, [action("Ask Dana to play Thursday")]);
    if (!once.ok || !twice.ok) throw new Error("unreachable");
    expect(once.value).toEqual(twice.value);
  });

  test("the id carries no trace of the name, so correcting one does not orphan the other", () => {
    const result = normalize(EMPTY, [action("Ask Dana to play Thursday")]);
    if (!result.ok) throw new Error("unreachable");
  ...
  test("two identical names in one batch get distinct ids", () => {
    const result = normalize(EMPTY, [action("Ask Dana"), action("Ask Dana")]);
    if (!result.ok) throw new Error("unreachable");
  ...
  test("minting avoids ids already committed", () => {
    const head = seeded([action("Ask Dana")]);
    const result = normalize(head, [action("Ask Dana")]);
    if (!result.ok) throw new Error("unreachable");
    const id = result.value[0]?.op === "add_node" ? result.value[0].id : "";
    expect([...head.nodes.keys()]).not.toContain(id);
  });

  test("a name with nothing in the id alphabet still mints", () => {
    // A slug had to fall back to a literal "activity" here; a hash does not care.
    const result = normalize(EMPTY, [action("— — —")]);
    if (!result.ok) throw new Error("unreachable");
    const id = result.value[0]?.op === "add_node" ? result.value[0].id : "";
    expect(id).toMatch(/^t-[0-9a-z]{4}$/);
  });
...
describe("$N resolution", () => {
  ...
  test("resolves refs inside an activity spec, not only at the top level", () => {
    const result = normalize(EMPTY, [
      action("Send invite"),
      acceptEvent("Await reply", { deadline: { after: "$0", duration: "48h" } }),
      action("Undo invite", { compensates: "$0" }),
    ]);
    if (!result.ok) throw new Error("unreachable");
    const awaited = result.value[1];
    const undo = result.value[2];
    // Every `$0` here means the first activity, so they all have to land on the id it minted.
    const sendInvite = result.value[0]?.op === "add_node" ? result.value[0].id : "";
    expect(
      awaited?.op === "add_node" && awaited.type === "accept_event" ? awaited.spec.deadline : null,
    ).toEqual({ after: sendInvite, duration: "48h" });
    expect(undo?.op === "add_node" ? undo.spec.compensates : null).toBe(sendInvite);
  });

  test("rejects a forward reference", () => {
    expectRefusal(EMPTY, [{ op: "add_edge", from: "$1", to: "$0" }, action("A")], "FORWARD_REF");
  });

  test("rejects a self reference", () => {
    expectRefusal(EMPTY, [action("A", { compensates: "$0" })], "FORWARD_REF");
  });
  ...
  test("rejects a ref to an op that minted no id", () => {
    expectRefusal(
      ...
      [
        action("A"),
        { op: "set_status", node: "$0", status: "completed", evidence_ref: "e" },
  ...
  test("rejects a committed id that does not exist — never invent an id", () => {
    ...
    expectRefusal(
      seeded([action("A")]),
      [{ op: "supersede_node", node: "a", by: "ghost" }],
      "UNKNOWN_ACTIVITY",
    );
    expectRefusal(EMPTY, [action("A", { compensates: "ghost" })], "UNKNOWN_ACTIVITY");
  });

  test("a supersede with no replacement stays shaped that way", () => {
    const head = seeded([action("A")]);
