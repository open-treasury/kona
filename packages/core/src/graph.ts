 ...
 *
 * Activity order is insertion order and edge order is append order, so two folds of the same
...
import type { EdgeCondition, ActivityType, Status, Verdict } from "./vocab.ts";
...
export function emptyGraph(schemaVersion: number): Graph {
  return { schema_version: schemaVersion, version: 0, activities: new Map(), edges: [] };
}

export function activityIds(graph: Graph): Set<string> {
  return new Set(graph.activities.keys());
}
...
export function isActivityTerminal(activity: Activity): boolean {
 ...
 */
export function satisfiesBlockingEdge(activity: Activity): boolean {
 ...
 */
export function resolutionOf(activity: Activity): EdgeCondition | null {
 ...
 * rewrite an activity that is already terminal, or one that is `sending`, so those activities sit on a
 ...
 * included". A `sending` activity completing later makes no edge newly dead, so no commit-time
...
/**
 * §6.4 — "Readiness fails safe." An activity is ready iff it is not dropped and every blocking
 ...
 * never satisfies readiness. Otherwise the second activity on an untaken branch has no
 ...
 * conditioned in-edge from `wait-for-priya`, which `supersede_activity` dropped. Nothing can
 ...
 */
export function isReady(graph: Graph, activity: Activity): boolean {
  if (activity.status.state !== "active") return false;
  if (activity.provenance.superseded_by !== null) return false;
  ...
  // leaves a terminal or `sending` activity on a dead arm exactly as it found it, so `dropped`
  // alone would let one of those satisfy a `merge: "any"` join on a branch nobody took.
  const live = ins.filter(
    (edge) =>
      graph.activities.get(edge.from)?.status.state !== "dropped" && !isArmDead(graph, edge.from),
  );
  ...
  return activity.spec.merge === "any"
...
export function readyFrontier(graph: Graph): Activity[] {
  return [...graph.activities.values()].filter((activity) => isReady(graph, activity));
}
...
export function projectGraph(graph: Graph): GraphProjection {
  return {
    ...
    activities: [...graph.activities.values()],
    edges: graph.edges,
  };
}
