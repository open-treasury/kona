/**
 * How a node is named in a message: its label, with the id that addresses it.
 *
 * Ids are hashes, so an id alone tells a reader nothing about which step is being refused.
 * The label alone would be ambiguous — two steps can share one — so both, always in the same
 * order, so the shape is learnable.
 */
export function named(node: { id: string; label?: string }): string {
  return node.label === undefined ? `'${node.id}'` : `'${node.label}' (${node.id})`;
}
 ...
 * UNKNOWN_NODE case, where there is no label to give and the id is the whole point.
 */
export function namedIn(
  graph: { nodes: Map<string, { id: string; label: string }> },
  id: string,
): string {
...
export function namedHere(
  graph: { nodes: Map<string, { id: string; label: string }> },
  ops: readonly { op: string; id?: string; label?: string }[],
  id: string,
): string {
  const committed = graph.nodes.get(id);
  if (committed !== undefined) return named(committed);
  for (const op of ops) {
    if (op.op === "add_node" && op.id === id && op.label !== undefined) return named({ id, label: op.label });
  }
  return `'${id}'`;
}
