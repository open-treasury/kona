 ...
 *
 * `fixtures/thursday.*` is a fixed historical log migrated to schema v6 by
 * `./scripts/make-fixture.sh`. The real CLI folds and projects it, but the current topology
 * validator did not admit its records. That makes it a read-compatibility and viewer-story
 * fixture; `fixtures/goalie.*` is the native v6 topology fixture.
 */
...
/** `kona graph --json` at head, as the binary projected it — the read contract, on disk. */
 ...
 * until v11, which is what leaves her activity `active` across four versions.
 */
export const V = {
  ...
  priyaFailed: 11,
  /** Priya's wait is withdrawn and Pat's arm is planned. */
  patPlanned: 12,
...
/**
 * The id the fixture carries for a node, found by its name.
 *
 * Ids are hashes, so a test cannot spell one from the person's name. The name is the stable
