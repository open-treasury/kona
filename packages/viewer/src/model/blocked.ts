...
import type { Edge, Graph, Activity } from "@kona/core";
 ...
 * activity's standing rather than its state: a superseded activity can still be `active`, and the
 ...
 * replacement. And an activity that is `done` *and* superseded — which is what the fixture's
 ...
 */
export function readinessOf(graph: Graph, activity: Activity): Readiness {
  if (activity.provenance.superseded_by !== null) return "superseded";
  if (isTerminal(activity.status.state)) return "settled";
  if (activity.status.state === "in_flight") return "running";
  if (isReady(graph, activity)) return "ready";
  return "blocked";
}

/**
 * One cause per unsatisfied in-edge, in edge order, or null when the activity is not blocked.
 *
 ...
 * reader's place.
 */
export function blockedReason(graph: Graph, activity: Activity): BlockedReason | null {
  if (readinessOf(graph, activity) !== "blocked") return null;

  const edges = inEdges(graph, activity.id);
  const causes = edges
  ...
  return {
    ...
    // in-edge to be satisfied: the activity can never reach the frontier again, whatever the
    // other blockers do next. `every` would be the right answer for a `merge:"any"` activity —
...
function causeFor(graph: Graph, edge: Edge): BlockedCause {
  ...
  if (source === undefined) {
    // No activity, so no label: the id is the only name we can honestly print.
    return {
      from: edge.from,
      fromLabel: edge.from,
      wants,
...
function pending(source: Activity): string {
...
function mismatch(source: Activity, fired: string | null, wants: string | null): string {
 ...
 * no resolution yet is a different matter: `record_outcome` is legal against a terminal activity
