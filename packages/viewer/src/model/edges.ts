 ...
 * activity ready or blocked. The distinction is carried in `kind` and paid off in how each one is
...
export interface ViewEdge {
   ...
   *  joined twice (a wait can depend on the activity it also escapes to). */
  id: string;
  from: string;
  to: string;
  kind: EdgeKind;
 ...
 *
 * The activity-diagram vocabulary — a filled dot for the initial activity, a ringed dot for the
 ...
 * changes mid-run, so an activity with nothing depending on it today grows children tomorrow; that
