...
test("release assembly rejects an omitted copy resource and copy version skew", async () => {
  ...
  try {
    ...
    await writeFile(copyManifest, original.replace('"version": "0.3.0"', '"version": "9.9.9"'));
...
test("installer verifies and atomically installs an immutable version, then is a no-op", async () => {
  ...
  try {
    ...
    assert.match(installed.stdout, /installed kona 0\.3\.0/);
    const link = join(value.bin, "kona");
    const target = await readlink(link);
    assert.equal(target, join(value.home, "data/kona/versions/v0.3.0/bin/kona"));
    ...
    assert.match(requests, /\/releases\/download\/v0\.3\.0\/SHA256SUMS/);
    assert.match(requests, /\/releases\/download\/v0\.3\.0\/kona-v0\.3\.0-portable\.tar\.gz/);
...
test("installer refuses unapproved redirects before contacting them", async () => {
  ...
  try {
    ...
    assert.match(
      requests[0],
      /^https:\/\/github\.com\/open-treasury\/kona\/releases\/download\/v0\.3\.0\//,
    );
  } finally {
    await value.cleanup();
  }
...
test("checksum failure and an unowned destination preserve existing files", async () => {
  ...
  try {
    ...
    const archive = join(value.release, "kona-v0.3.0-portable.tar.gz");
...
test("installer rejects an internally corrupted archive after its external checksum passes", async () => {
  ...
  try {
    ...
    await writeFile(
      ...
      `${installerHash}  install.sh\n${archiveHash}  kona-v0.3.0-portable.tar.gz\n`,
    );
    const refused = await install(value);
    assert.notEqual(refused.code, 0);
...
test("tampered owned versions are refused without changing the active link", async () => {
  ...
  try {
    ...
    const skill = join(value.home, "data/kona/versions/v0.3.0/skills/copy/SKILL.md");
    await chmod(skill, 0o644);
...
test("a verified upgrade preserves the old immutable version and atomically activates the new one", async () => {
  ...
  try {
    ...
    const next = await buildVersion(value, "0.4.0");
    ...
    assert.match(upgraded.stdout, /installed kona 0\.4\.0/);
    assert.equal(
      await readlink(join(value.bin, "kona")),
      join(value.home, "data/kona/versions/v0.4.0/bin/kona"),
    );
...
test("installer refuses a downgrade and preserves the newer active release", async () => {
  const value = await fixture();
  try {
    const newer = await buildVersion(value, "0.4.0");
    ...
    const refused = await install(value);
    assert.notEqual(refused.code, 0);
    assert.match(refused.stderr, /refusing downgrade from 0\.4\.0 to 0\.3\.0/);
    assert.equal(await readlink(link), active);
...
test("activation failure leaves the prior release active and records no false ownership", async () => {
  ...
  try {
    ...
    assert.equal(await readlink(link), join(value.home, "data/kona/versions/v0.4.0/bin/kona"));
  } finally {
    await value.cleanup();
  }
});
...
test("concurrent installers serialize activation and never remove the installed winner", async () => {
  ...
  try {
    ...
    assert.deepEqual(Object.keys(state.versions), ["v0.3.0"]);
  } finally {
    await value.cleanup();
  }
});
