...
case "\${KONA_TEST_REDIRECT:-none}:$url" in
  ...
  *)
    ...
    case "$url" in
      */releases/download/v\${KONA_TEST_CURRENT_VERSION:-0.5.3}/*) source_release=\${KONA_TEST_CURRENT_RELEASE:-$source_release} ;;
    esac
...
`,
  ...
  const env = {
    ...
    KONA_TEST_CURRENT_VERSION: "0.5.3",
...
test("release assembly rejects an omitted copy resource and copy version skew", async () => {
  ...
  try {
    ...
    await writeFile(copyManifest, original.replace('"version": "0.5.3"', '"version": "9.9.9"'));
...
test("release assembly rejects SemVer components with leading zeroes", async () => {
  ...
  try {
    ...
    await writeFile(
      packagePath,
      (await readFile(packagePath, "utf8")).replace('"version": "0.5.3"', '"version": "01.0.0"'),
    );
...
test("installer verifies and atomically installs an immutable version, then is a no-op", async () => {
  ...
  try {
    ...
    assert.match(installed.stdout, /installed kona 0\.5\.3/);
    const link = join(value.bin, "kona");
    const target = await readlink(link);
    assert.equal(target, join(value.home, "data/kona/versions/v0.5.3/bin/kona"));
    ...
    assert.match(requests, /\/releases\/download\/v0\.5\.3\/SHA256SUMS/);
    assert.match(requests, /\/releases\/download\/v0\.5\.3\/kona-v0\.5\.3-portable\.tar\.gz/);
...
test("authenticated --latest verifies the discovered installer and runs normal mode quietly", async () => {
  ...
  try {
    ...
    assert.equal(
      ...
      join(value.home, "data/kona/versions/v0.5.3/bin/kona"),
    );
    const requests = (await readFile(join(value.directory, "requests.log"), "utf8"))
      .trim()
      .split("\n");
    ...
    assert.ok(
      requests.includes(
        "https://github.com/open-treasury/kona/releases/download/v0.5.3/SHA256SUMS",
      ),
    );
  } finally {
    await value.cleanup();
...
test("installer refuses unapproved redirects before contacting them", async () => {
  ...
  try {
    ...
    assert.match(
      requests[0],
      /^https:\/\/github\.com\/open-treasury\/kona\/releases\/download\/v0\.5\.3\//,
    );
  } finally {
    await value.cleanup();
  }
...
test("checksum failure and an unowned destination preserve existing files", async () => {
  ...
  try {
    ...
    const archive = join(value.release, "kona-v0.5.3-portable.tar.gz");
...
test("installer rejects an internally corrupted archive after its external checksum passes", async () => {
  ...
  try {
    ...
    await writeFile(
      join(value.release, "SHA256SUMS"),
      `${installerHash}  install.sh\n${archiveHash}  kona-v0.5.3-portable.tar.gz\n`,
    );
    const refused = await install(value);
    assert.notEqual(refused.code, 0);
...
test("tampered owned versions are refused without changing the active link", async () => {
  ...
  try {
    ...
    const skill = join(value.home, "data/kona/versions/v0.5.3/skills/copy/SKILL.md");
    await chmod(skill, 0o644);
...
test("installer refuses a downgrade and preserves the newer active release", async () => {
  ...
  try {
    ...
    const refused = await install(value);
    assert.notEqual(refused.code, 0);
    assert.match(refused.stderr, /refusing downgrade from 0\.5\.3 to 0\.5\.0/);
    assert.equal(await readlink(link), active);
...
test("concurrent installers serialize activation and never remove the installed winner", async () => {
  ...
  try {
    ...
    const link = join(value.bin, "kona");
    assert.equal(await readlink(link), join(value.home, "data/kona/versions/v0.5.3/bin/kona"));
    ...
    assert.deepEqual(Object.keys(state.versions), ["v0.5.3"]);
  } finally {
    await value.cleanup();
  }
});
