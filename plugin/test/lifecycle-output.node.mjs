...
const success = (verb, host, scope, status, version = "0.5.0") => ({
  ok: true,
  verb,
  host,
  scope,
...
test("human lifecycle successes use consistent host-aware sentences", () => {
  const cases = [
    [
      ...
      "Installed Kona v0.5.0 for OpenCode (project).",
    ],
    [success("update", "codex", "user", "updated"), "Updated Kona to v0.5.0 for Codex (user)."],
    [
      success("verify", "claude", "local", "active"),
      "Verified Kona v0.5.0 for Claude Code (local).",
    ],
    ...
    [
      ...
      "Already up to date: Kona v0.5.0 for Pi (user).",
    ],
    [
