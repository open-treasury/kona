...
checksum_for() {
  ...
  awk -v name="$checksum_name" '
    ...
    END { if (count == 1 && valid == 1) print hash }
  ' "$checksum_file"
}
...
case "${1:-}" in
  '') ;;
  --latest)
    [ "$#" -eq 1 ] || fail '--latest accepts no additional arguments'
    latest_installer="$work/install.sh"
    fetch "$KONA_PUBLIC_INSTALL_URL" "$latest_installer"
    latest_version=$(awk '
      /^[[:space:]]*KONA_VERSION[[:space:]]*=/ { count++ }
      /^KONA_VERSION='\''(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)'\''$/ {
        valid++
        value=$0
        sub(/^KONA_VERSION='\''/, "", value)
        sub(/'\''$/, "", value)
      }
      END { if (count == 1 && valid == 1) print value }
    ' "$latest_installer")
    [ -n "$latest_version" ] || fail 'latest installer must contain exactly one valid KONA_VERSION assignment'
    latest_checksums="$work/SHA256SUMS"
    fetch "https://github.com/open-treasury/kona/releases/download/v${latest_version}/SHA256SUMS" "$latest_checksums"
    expected_installer_checksum=$(checksum_for "$latest_checksums" 'install.sh')
    [ -n "$expected_installer_checksum" ] || fail 'SHA256SUMS must contain exactly one valid install.sh record'
    actual_installer_checksum=$(sha256_file "$latest_installer")
    [ "$actual_installer_checksum" = "$expected_installer_checksum" ] || fail 'installer checksum verification failed'
    if [ "$(compare_versions "$latest_version" "$KONA_VERSION")" -lt 0 ]; then
      KONA_INSTALL_ALLOW_NEWER_NOOP=1 sh "$0" >/dev/null || fail 'verified current installation failed'
    else
      KONA_INSTALL_ALLOW_NEWER_NOOP=1 sh "$latest_installer" >/dev/null || fail 'verified latest installation failed'
    fi
    exit 0
    ;;
...
expected_checksum=$(checksum_for "$checksums" "$KONA_ARCHIVE")
[ -n "$expected_checksum" ] || fail "SHA256SUMS must contain exactly one valid $KONA_ARCHIVE record"
...
if (binExists) {
  ...
  if (compareVersions(version, activeTag.slice(1)) < 0) {
    if (process.env.KONA_INSTALL_ALLOW_NEWER_NOOP === "1") process.exit(0);
