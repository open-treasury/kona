...
**The one exception, and we found it the hard way.** An empirical run of the mutator at n=60 (`docs/probes/q4-mutator-v3.md`) found that when it could not satisfy a constraint, it **invented counterparties and queued real email to them** — passing every structural check, because the cheapest way to satisfy "we need one more goalie" is to add a candidate and nothing required the candidate to be real. So: **introducing a recipient the graph has never seen requires a human.** Everything else stays automatic. The plan changes freely; the world does not; and nobody new enters the world without a human.
