...
**Status: Historical, superseded for schema and implementation guidance** (2026-08-28) · **Original approval:** 2026-08-21 · **Current contract:** [`spec.md`](./spec.md), schema v6
**Convention:** one module = one epic · every task ≤ 4h human effort · AI effort estimated separately and more atomically.

> This is the executed schema-v1-v5 build plan and retains its original task estimates, vocabulary, and migration record as history. Do not use its `task`/`wait`, five-status, `scope`, `condition`, or `sending` examples to author schema-v6 data; use `spec.md` instead.
...
_The substrate. Everything blocks on this; nothing here is optional._

| ID       | Task                                                                                                                                                                         | Human |   AI | Deps |
| -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----: | ---: | ---- |
| T1.1     | **Monorepo scaffold** — Bun workspace, **3 packages** per §6.12 (+ `demo/`, `plugin/`), `tsconfig` strict + project refs, lint, test runner, cycle check in `build`          | 1.5 h | 30 m | —    |
| T1.2     | **`packages/core`** — 2 node types, **1 edge kind**, 5 statuses, the edge record, `status`/`outcome`/`output`, typed deadlines (3 shapes). **Zero deps; unblocks W3 and W4** |   2 h | 30 m | T1.1 |
...
_Still the largest epic, though 5 h lighter after the invariant cut. Carries every probe finding. **T2.6 is not optional** — it is what four probe runs converged on._
...
_Where the "nobody else has a clock or a counterparty" claim actually lives._
...
**The unblocking moment — reached 2026-08-21.** T2.1, T2.7 and T2.8 are closed and T4.1 is partial: `kona init`, `kona mutate` and `kona graph --json` work end to end against invariant 1 alone. Invariants 2 and 3 were _deliberately_ left for later, which is the point — **building all the invariants first is the biggest available scheduling error**, and it keeps E6/E7 idle for hours.

`fixtures/thursday.*` exists so W3 and W4 need nothing further from W1/W2: one fixed historical pursuit, migrated to schema v6 and folded by the real binary. It is not a native topology-validation fixture; `fixtures/goalie.*` fills that role.

### Window allocation after the unblock

| Window       | Owns                                                                                                                                       | Runs until                       |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------- |
| **W1**       | `core` (types, ops, invariants, branch resolution — pure, no I/O, no model)                                                                | the spine is done                |
| **W2**       | `kona` (fold, lock+CAS, waits, outbox, resume, the 9 verbs — store+effects+cli merged in §0.6/§0.7)                                        | resume passes `kill -9`          |
| **W3**       | E6 viewer — needed only **`packages/core` to compile** (§0.8 merged `schema`+`engine` into it), not a working `graph --json`. **Open now** | timeline panel; scrubber if time |
| **W4**       | E7 demo rig — needs **`packages/core` + the port interface** only. **Open now**                                                            | divergence script runs           |
...
| 4     | `kona lint` rules L1–L5             | They catch _plan_ defects before approval; the store still blocks every corruption path                                                                 |
...
- **§0.8 pass four.** Edge kinds 3→1 (provenance is a node field, not an edge — it was represented twice). Edge record 6 fields→3, losing identity entirely. `parent_v` deleted from the record. Packages 4→3 (`schema`+`engine`→`core`). ~32 h → ~28 h. **The algorithm has converged: 57%→37%→29%→12%. Stop cutting; start building.**
