...
**The unblocking moment — reached 2026-08-21.** T2.1, T2.7 and T2.8 are closed and T4.1 is partial: `kona init`, `kona mutate` and `kona graph --json` work end to end against invariant 1 alone. Invariants 2 and 3 were *deliberately* left for later, which is the point — **building all the invariants first is the biggest available scheduling error**, and it keeps E6/E7 idle for hours.

`fixtures/thursday.*` exists so W3 and W4 need nothing further from W1/W2: one pursuit, eight versions, all five statuses, generated by the real binary.
