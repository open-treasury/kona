...
- **Hero asset:** `docs/img/viewer.jpg` — a JPEG capture of a real pursuit in the activity model: an initial node,
  a fork, two arms of _action → accept-event → decision_, guarded routes to a merge or
  flow-final, a join, and the final ring. Re-shot 2026-08-28 from
  `fixtures/goalie.*`; `docs/img/viewer-v5.png` keeps the pre-redesign render, which cannot be
  regenerated because its source log is no longer foldable. Full-bleed on slide 3, the single
...
- **Type:** a real display face for headlines paired with a monospace for anything literal from
  the system (`active`, `CONTRADICTION`, `kona next`, `UNEVIDENCED_RECIPIENT`). The line
  between prose and machine vocabulary should be visible at a glance.
- **Colour:** one accent, spent on the claimed `active` state and on refusals — so the eye
...
1. the spinning claimed node → `active`
...
| **The timeline is irreversible** | AFlow / ADAS / DSPy optimise _between_ runs, by re-executing them            |
...
**Vocabulary is literal.** `active`, `CONTRADICTION`, `kona next`, `supersede_node`,
...
- If asked for the score in Q&A: _we ran it, the variance swamped the effect at n=1, and the
  rig is in the repo so it can be run properly._
